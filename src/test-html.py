...
if len(quotes) == 0:
    print
    "Couldn't connect to yahoo trading database"
else:
    ...
    for j in range(1, 7):
        ...
        if i == j:
            ...
            sp = [Histogram(x=x, histnorm='probability density'),
                  Scatter(x=x_grid, y=kde_scipy(x.as_matrix(), x_grid), line=Line(width=2, color='red', opacity='0.5'))]
        else:
            sp = [Scatter(x=df[x_ticker], y=df[y_ticker], mode='markers', marker=Marker(size=3))]

        for ea in sp:
            ea.update(name='{0} vs {1}'.format(x_ticker, y_ticker), xaxis='x{}'.format(subplots[sp_index]),
